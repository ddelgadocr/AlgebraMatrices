...
public Matriz Cofactores2(Matriz matriz){
    Fraction[][] elementos = new Fraction[matriz.cantidadFilas()][matriz.cantidadColumnas()];
    Matriz cofactores = new Matriz(elementos);
    Fraction unon= new Fraction(-1, 1);
    
    cofactores.setElemento(0, 0, matriz.getElemento(1, 1));
    cofactores.setElemento(0, 1, matriz.getElemento(1, 0).multiply(unon));
    cofactores.setElemento(1, 0, matriz.getElemento(0, 1).multiply(unon));
    cofactores.setElemento(1, 1, matriz.getElemento(0, 0));
    return cofactores;
}

public Matriz AdjuntaMatriz2(Matriz matriz){    
    return Transpuesta(Cofactores2(matriz));
}
        ...
        for (int i = 0; i < minversa.cantidadFilas(); i++) {
            for (int j = 0; j < minversa.getColumnas(); j++) {
                if(matriz.Determinante(matriz.cantidadFilas()) != cero){
                    minversa.setElemento(i, j,uno.divide(matriz.Determinante(matriz.cantidadFilas())).multiply(AdjuntaMatriz2(matriz).getElemento(i, j)));
                }
            }
        }
        minversa.imprime();
